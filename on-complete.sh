...
if [[ $2 -eq 1 ]]; then # single file
	...
	rclone -v --config="rclone.conf" --delete-empty-src-dirs --no-traverse move "$3" "$RCLONE_DESTINATION" 2>&1	
elif [[ $2 -gt 1 ]]; then # multiple file
	#rclone -v --config="rclone.conf" --no-traverse copy "$topPath" "$RCLONE_DESTINATION/${relativePath%%/*}"
	rclone -v --config="rclone.conf" --delete-empty-src-dirs --no-traverse move "$topPath" "$RCLONE_DESTINATION/${relativePath%%/*}"
fi
